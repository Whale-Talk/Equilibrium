...
def run_backtest_for_years():
    # 读取K文件夹数据
    conn_k = sqlite3.connect('/home/mjy/AI量化/K/data/btc_klines.db')
    
    for year in [2019, 2020, 2021, 2022, 2023, 2024, 2025]:
        # 获取年度数据
        if year == 2019:
            start_ts = 1546272000000  # 2019-01-01
        else:
        ...
        if year == 2025:
            ...
            end_ts = int(datetime(2026, 2, 26).timestamp() * 1000)
        else:
            end_ts = int(datetime(year+1, 1, 1).timestamp() * 1000) - 1
        
        cursor = conn_k.cursor()
        cursor.execute(f"""
